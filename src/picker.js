...
let formats={hex:true,rgb:true,hsl:false,cmyk:false},currentColor=null,pendingPoint=null,inFlight=false,timer=null,lastRequest=0,confirming=false,cardWidth=170,cardHeight=80,latestPosition=null,positionFrame=0,pointerTimer=null,stopped=false,motionVersion=0;
...
async function runSample(){if(inFlight||!pendingPoint)return;const point=pendingPoint;pendingPoint=null;inFlight=true;lastRequest=performance.now();try{renderColor(await invoke('sample_color',{windowLabel,localX:point.x,localY:point.y,viewWidth:innerWidth,viewHeight:innerHeight}));positionCard(point.x,point.y)}catch(error){name.textContent=String(error)}inFlight=false;if(pendingPoint)scheduleSample()}
...
function queuePoint(event){motionVersion++;pendingPoint={x:event.clientX,y:event.clientY};scheduleSample()}
...
async function init(){document.body.focus();const state=await invoke('get_state');formats=state.formats||formats;if(state.theme!=='system')document.documentElement.dataset.theme=state.theme;await pollPointer()}
