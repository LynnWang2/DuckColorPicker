...
let appState = { shortcut: '', history: [], theme: 'system', formats: {} };
let latest;
...
function render(state) {
  appState=state;setTheme(state.theme);$('#shortcut').value=humanShortcut(state.shortcut);$('#shortcutLabel').textContent=humanShortcut(state.shortcut);$('#theme').value=state.theme;$('#autoStart').checked=state.autoStart;
...
async function init(){
  ...
  await listen('picker-result',event=>{const color=event.payload;latest=color;$('#latest').classList.remove('hidden');$('#latestSwatch').style.background=color.hex;$('#latestName').textContent=color.name;$('#latestValues').innerHTML=valueRows(color);$('#hint').textContent='已复制 HEX 色值（不含 #），可继续取色';toast(`${copyValue(color.hex)} · ${color.name}`)});
